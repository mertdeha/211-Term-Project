...
public:
  static void generateWebInterface(DynamicArray<Course *> &allCourses, 
                                   ...
                                   int totalSlots) {
    // Dosya adini Ingilizce yaptik
    std::ofstream htmlFile("exam_schedule.html");
    ...
    htmlFile << "<tr><th>Course Code</th><th>Course Name</th><th>Assigned Room(s)</th></tr>\n";

    for (int currentSlot = 0; currentSlot < totalSlots; currentSlot++) {
      htmlFile << "<tr class='slot-row'><td colspan='3'>TIME SLOT " << currentSlot + 1 << "</td></tr>\n";
      ...
      for (int i = 0; i < allCourses.size(); i++) {
        ...
        if (assignedSlot != NULL && *assignedSlot == currentSlot) {
            ...
            std::string assignedRoom = (roomPtr != NULL) ? *roomPtr : "Not Assigned";

            htmlFile << "<tr><td>" << courseId << "</td><td>" << allCourses.get(i)->name << "</td><td>" << assignedRoom << "</td></tr>\n";
            hasExams = true;
        }
      }
      if (!hasExams) htmlFile << "<tr><td colspan='3' style='text-align:center; color:#7f8c8d;'>No exams in this time slot.</td></tr>\n";
    }

    htmlFile << "</table>\n</div>\n</body>\n</html>\n";
    htmlFile.close();
    
    std::cout << "\n[FRONTEND] Web interface successfully generated as 'exam_schedule.html'!\n" << std::endl;
  }
};
#endif