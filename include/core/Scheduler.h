...
public:
  ...
  static bool colorGraph(DynamicArray<Course *> &allCourses, Graph &conflictGraph,
                         int totalAvailableSlots, HashMap<std::string, int> &courseToSlotMap) {
    ...
    for (int slot = 0; slot < totalSlots; slot++) {
      ...
      for (int i = 0; i < coursesInThisSlot.size() - 1; i++) {
        for (int j = 0; j < coursesInThisSlot.size() - i - 1; j++) {
          if (coursesInThisSlot.get(j)->enrolledStudents.getSize() < coursesInThisSlot.get(j + 1)->enrolledStudents.getSize()) {
